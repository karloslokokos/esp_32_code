...
const char* deviceID = "ESP32_8"; 

const char* distanceTopic = "ultrasonic_distance_8";  // MQTT topic for distance data
const char* motionTopic = "Esp32_7_triggered";        // MQTT topic for motion detection
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    if (distanceCm < MAX_DISTANCE) {
      if (!beamBroken) {
        ...
        if (client.publish(motionTopic, "B3")) {
          Serial.println("note_8_detected, Message sent.");
          beamBroken = true;
        } else {
