...
const char* deviceID = "ESP32_4"; 

const char* distanceTopic = "ultrasonic_distance_4";  // MQTT topic for distance data
const char* motionTopic = "Esp32_4_triggered";        // MQTT topic for motion detection
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    if (distanceCm < MAX_DISTANCE) {
      if (!beamBroken) {
        ...
        if (client.publish(motionTopic, "F3")) {
          Serial.println("note_4_detected, Message sent.");
          beamBroken = true;
        } else {
