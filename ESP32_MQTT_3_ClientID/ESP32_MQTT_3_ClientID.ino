...
const char* deviceID = "ESP32_3"; 

const char* distanceTopic = "ultrasonic_distance_3";  // MQTT topic for distance data
const char* motionTopic = "Esp32_3_triggered";        // MQTT topic for motion detection
const char* topic = "sensor_data";                  // MQTT topic for other sensor data
const int trigPin = 10;
const int echoPin = 11;
const int MAX_DISTANCE = 65;
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    if (distanceCm < MAX_DISTANCE) {
      if (!beamBroken) {
        ...
        if (client.publish(motionTopic, "E3")) {
          Serial.println("note_3_detected, Message sent.");
          beamBroken = true;
        } else {
