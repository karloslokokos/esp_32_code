...
const char* distanceTopic = "ultrasonic_distance_2";  // MQTT topic for distance data
const char* motionTopic = "note_2_detected";        // MQTT topic for motion detection
...
const long interval = 1000;  // Set the interval for sensor readings (1 second)
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    if (distanceCm < MAX_DISTANCE) {
      if (!beamBroken) {
        ...
        if (client.publish(motionTopic, "note_2_detected")) {
          Serial.println("note_2_detected, Message sent.");
          beamBroken = true;
        } else {
